...
function Dashboard({navigation,root}:any){
 ...
 return <ScrollView style={s.page} contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={q.busy} onRefresh={q.load} tintColor={C.gold}/>}>
  ...
  <View style={s.metricGrid}>{metrics.length?metrics.map(([k,v])=><View style={s.metric} key={k}><Text style={s.metricValue}>{String(v)}</Text><Text style={s.metricLabel}>{k.replace(/([A-Z])/g," $1").trim()}</Text></View>):<Empty title="Live overview" copy="Your business activity will appear here as bookings and payments grow."/ >}</View>
...
function Services({navigation}:any){
 ...
 return <ScrollView style={s.page} contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={q.busy} onRefresh={q.load} tintColor={C.gold}/>}>
  ...
  {!q.busy&&!q.data?.length?<Empty title="No services yet" copy="Create your first service with pricing, availability and a work gallery."/>:null}
...
function ServiceEditor({route,navigation}:any){
 ...
 const[name,setName]=useState(""),[description,setDescription]=useState(""),[basePrice,setBasePrice]=useState(""),[categoryId,setCategoryId]=useState(""),[priceType,setPriceType]=useState("fixed"),[city,setCity]=useState(""),[state,setState]=useState(""),[images,setImages]=useState<any[]>([]);
 useEffect(()=>{(async()=>{try{const cats=listOf(await endpoints.categories());setCategories(cats);if(!categoryId&&cats[0])setCategoryId(idOf(cats[0]));if(serviceId){const x:any=unwrap(await endpoints.service(serviceId));setName(x?.name??"");setDescription(x?.description??"");setBasePrice(String(x?.basePrice??""));setCategoryId(String(x?.categoryId?._id??x?.categoryId??""));setPriceType(x?.priceType??"fixed");setCity(x?.location?.city??"");setState(x?.location?.state??"");setImages((x?.images??[]).map((uri:string,index:number)=>({uri,name:`Service image ${index+1}`,remote:true,index})))}}catch(e){setError(errMsg(e))}finally{setBusy(false)}})()},[serviceId]);
 const pickImages=async()=>{try{const remaining=12-images.length;if(remaining<=0)return Alert.alert("Gallery full","A service can have up to 12 images.");const pick=await DocumentPicker.getDocumentAsync({type:["image/jpeg","image/png","image/webp"],multiple:true,copyToCacheDirectory:true});if(pick.canceled)return;setImages(current=>[...current,...pick.assets.slice(0,remaining).map((a:any)=>({uri:a.uri,name:a.name,type:a.mimeType||"image/jpeg",remote:false}))])}catch(e){Alert.alert("Images",errMsg(e))}};
 const removeImage=async(index:number)=>{const image=images[index];try{if(image?.remote&&serviceId)await endpoints.deleteServiceImage(serviceId,image.index??index);setImages(current=>current.filter((_,i)=>i!==index))}catch(e){Alert.alert("Image not removed",errMsg(e))}};
 const uploadNewImages=async(id:string)=>{const pending=images.filter(x=>!x.remote);if(!pending.length)return;const form=new FormData();pending.forEach((a:any)=>form.append("files",{uri:a.uri,name:a.name||"service.jpg",type:a.type||"image/jpeg"} as any));await endpoints.uploadServiceImages(id,form)};
 const save=async()=>{if(!name.trim()||description.trim().length<20||!categoryId||Number(basePrice)<0)return Alert.alert("Check service details","Name, category, price and a description of at least 20 characters are required.");setBusy(true);try{const body={name:name.trim(),description:description.trim(),categoryId,basePrice:Number(basePrice),priceType,tags:[],location:{city:city.trim(),state:state.trim(),serviceRadius:25},availability:{workingDays:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],startTime:"09:00",endTime:"18:00",blockedDates:[]},packages:[]};let saved:any;if(serviceId){saved=unwrap(await endpoints.updateService(serviceId,body))}else{saved=unwrap(await endpoints.createService(body))}const savedId=String(saved?._id??saved?.id??serviceId??"");if(savedId)await uploadNewImages(savedId);Alert.alert(serviceId?"Service updated":"Service created",serviceId?"Your service changes were saved.":"Your service and gallery were saved.");navigation.goBack()}catch(e){Alert.alert("Service not saved",errMsg(e))}finally{setBusy(false)}};
 ...
 return <ScrollView style={s.page} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled"><Header title={serviceId?"Edit service":"Add service"} onBack={navigation.goBack}/>{error?<Text style={s.error}>{error}</Text>:null}
  ...
  <Text style={s.label}>SERVICE GALLERY · {images.length}/12</Text><Text style={s.copy}>Add clear work photos. The first image is your cover image.</Text><ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{gap:10,marginTop:10}}>{images.map((img:any,index:number)=><View key={`${img.uri}-${index}`}><Image source={{uri:img.uri}} style={{width:110,height:110,borderRadius:16}} contentFit="cover"/><Pressable style={s.smallButton} onPress={()=>removeImage(index)}><Text style={s.smallButtonText}>Remove</Text></Pressable></View>)}</ScrollView><Pressable style={s.secondary} onPress={pickImages}><Text style={s.secondaryText}>Add service images</Text></Pressable>
