...
export function VendorScreen({onBack}:{onBack:()=>void}){
 ...
 const upload=async(type:string)=>{try{const pick=await DocumentPicker.getDocumentAsync({type:["image/jpeg","image/png","image/webp","application/pdf"],copyToCacheDirectory:true});if(pick.canceled)return;const asset=pick.assets[0];const form=new FormData();form.append("type",type);form.append("file",{uri:asset.uri,name:asset.name||`${type}.${asset.mimeType==="application/pdf"?"pdf":"jpg"}`,type:asset.mimeType||"application/octet-stream"} as any);await endpoints.uploadVendorDocument(form);Alert.alert("Upload complete",`${asset.name||type.replaceAll("_"," ")} is attached to your verification application.`);await load();}catch(e){Alert.alert("Upload failed",errMsg(e));}};
 ...
 {error?<><Text style={s.error}>{error}</Text><Pressable onPress={load} style={s.cta}><RefreshCw size={16}/><Text style={s.ctaText}>Retry</Text></Pressable></>:user?.role==="vendor"||user?.isVendorApproved?<><View style={s.hero}><BriefcaseBusiness size={25} color={C.goldLight}/><Text style={s.heroTitle}>Verified vendor workspace</Text></View><Text style={s.section}>OVERVIEW</Text>{Object.entries(overview??{}).filter(([,v])=>typeof v==="number").slice(0,6).map(([k,v])=><Text style={s.copy} key={k}>{k}: {String(v)}</Text>)}<Text style={s.section}>MY SERVICES</Text>{services.map((x,i)=><Text style={s.copy} key={String(x?._id??i)}>{x?.name} · {x?.status}</Text>)}</>:!showForm&&application?<><View style={s.hero}><FileCheck2 size={25} color={C.goldLight}/><Text style={s.heroTitle}>Application: {application.status}</Text><Text style={s.copy}>Review your identity, address and work proofs. Sensitive bank details stay masked after submission.</Text>{application.rejectionReason?<Text style={s.error}>{application.rejectionReason}</Text>:null}</View><Text style={s.section}>IDENTITY, ADDRESS & WORK PROOFS</Text>{["aadhaar","pan","gst","business_license","address_proof","profile_photo","shop_photo","portfolio","work_proof"].map(t=><Pressable key={t} style={s.upload} onPress={()=>upload(t)}><Upload size={17} color={C.goldLight}/><Text style={s.uploadText}>Upload {t.replaceAll("_"," ")}</Text></Pressable>)}{application.status==="rejected"?<Pressable style={s.cta} onPress={()=>setResubmit(true)}><Text style={s.ctaText}>Update and resubmit</Text></Pressable>:application.status!=="approved"?<Pressable style={s.cta} onPress={async()=>{try{setBusy(true);await endpoints.submitVendorKyc();Alert.alert("Application submitted","Your verification application is ready for OMIQORA review.");await load()}catch(e){Alert.alert("Submit application",errMsg(e))}finally{setBusy(false)}}}><Text style={s.ctaText}>Review & submit application</Text></Pressable>:null}</>:<><View style={s.hero}><MapPin size={25} color={C.goldLight}/><Text style={s.heroTitle}>Real vendor onboarding</Text><Text style={s.copy}>Set up your business profile, service location and payout details.</Text></View>
...
<View style={s.card}>
 ...
 <Text style={s.details}>{bookings.length.toLocaleString("en-IN")} bookings in your workspace.</Text>
...
<View style={s.card}>
 ...
 <Text style={s.details}>{payments.length.toLocaleString("en-IN")} transactions in your earnings history.</Text>
...
<View style={s.card}>
 ...
 <Text style={s.details}>{services.length.toLocaleString("en-IN")} services in your catalogue. Manage availability from each service.</Text>
